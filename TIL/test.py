...
class TestPlayAudio:
    ...
    def test_invalid_argument_filename(self):
        res = pibo.play_audio(out='local', volume='-2000', background=True)
...
class TestIsAlpha:
    ...
    """
    # isAlpha 현재 여러 요청값 중 하나만 alphabet이어도 True 출력. 에러 발생 요망.
...
class TestPointsCheck:
    ...
    # pibo.py (537line) [if points] => [if points != None]으로 변경 요망.
...
"""
# 실행 에러. 방법 좀 더 찾아보고 안되면 따로 테스트 형식을 바꾸자.
class TestStt:
    def test_valid(self):
        res = pibo.stt()
        assert res == res_form(True, "Success", "Success", res['data'])
...
class TestDeleteFace:
    ...
    def test_invalid_not_found_name(self):
        res = pibo.delete_face(name='notfounddb')
        assert res == res_form(ret, "NotFound error", "{} not exist in the facedb".format(name), None)

    def test_valid(self):
        res = pibo.delete_face(name='./facedb')
        assert res == res_form(ret, "Success", "Success", None)
...
class TestCheckFile:
    def test_valid(self):
        ...
        assert res == Path(filename).is_file()
...
class TestGetCodeMean:
    def test_valid(self):
        res = pibo.get_codeMean(errcode)
        assert res == res_form(True, "Success", "Success", n_list[errcode])

    def test_invalid_not_found(self):
        res = pibo.get_codeMean(errcode)
        assert res == res_form(False, "NotFound error", "Error code {} does not exist".format(errcode), Non)
